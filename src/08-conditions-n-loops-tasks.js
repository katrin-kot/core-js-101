...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(elem) === i && str.indexOf(elem, i + 1) === -1) {
      return elem;
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  const newstr = ccn.toString();
  const controlnum = Number(newstr[newstr.length - 1]);
  const arr1 = newstr.split('');
  arr1.splice(arr1.length - 1, 1);
  const arr = arr1.reverse();
  const twiceArr = arr.map((element, idx) => {
    if (idx % 2 === 0) {
      const twice = element * 2;
      if (twice > 9) {
        return (twice % 10) + 1;
      }
      return twice;
  ...
  const conNum = twiceArr.reduce((acc, cur) => acc + cur);
  const final = (conNum * 9) % 10;
  if (final === controlnum) {
    return true;
  }
  return false;
...
function getDigitalRoot(num) {
  ...
  const arr = str.split('');
  const sum = arr.reduce((acc, cur) => Number(acc) + Number(cur));
  if (sum > 9) {
...
function getCommonDirectoryPath(pathes) {
  ...
  const intermediateResult = pathes[0].substr(0, k);
  const idx = intermediateResult.lastIndexOf('/');
  if (idx === intermediateResult.length - 1) {
    return intermediateResult;
  }
  return intermediateResult.substr(0, idx + 1);
}
...
function getMatrixProduct(m1, m2) {
  ...
  const m3 = new Array(m1NumRows);
  for (let r = 0; r < m1NumRows; r += 1) {
    m3[r] = new Array(m2NumCols);
